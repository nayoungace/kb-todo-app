...
표와 코드 마커는 **파일별 개수가 일치해야 하며, `pnpm lint`가 이를 검사한다**(`scripts/check-mock-markers.mjs`). 타협 지점을 추가·삭제하면 표와 주석을 같은 커밋에서 함께 갱신한다. 규칙을 문서로만 두었을 때 실제로 어긋난 적이 있어(표 8행 / 마커 2건) 검사를 자동화했다.
