...
| 화면       | 라우트       | API            | 핵심 요구사항                                                                 |
| ---------- | ------------ | --------------------- | ----------------------------------------------------------------------------- |
| GNB/LNB    | -            | -                     | 대시보드, 할 일 라우트 링크(각 아이콘, 클릭 시 해당 페이지 이동), 로그인 상태에 따라 회원정보/로그인 아이콘 분기(클릭 시 각각 회원정보/로그인 페이지 이동) |
...
| 상세       | /task/:id  | [GET] /api/task/:id   | 상세 표시, 404 전용 화면(목록 복귀 버튼, 심미성 평가 대상 아님), 삭제 확인 모달  |
...
| 이름                 | 방식                          | 비고                                        |
| -------------------- | ----------------------------- | ------------------------------------------- |
| bearerAuth         | HTTP Bearer (JWT)             | sign-in, refresh 제외 전 API 적용 |
| refreshTokenCookie | Cookie token                | credentials 포함하여 요청     |

- 응답 헤더 명세 없음
- 전체 실패 응답의 body는 ErrorResponse (JSON)
- refresh token이 유효하지 않거나 만료된 경우 refresh 401 준다
- 별도 base URL 없이 same-origin(`/`)으로 요청
