...
[요구사항 요약](docs/requirements/summary.md)이 비어 있거나 해석이 갈리는 자리에서 내린 결정을 모았다. 코드에도 같은 내용을 `// 자의적 결정:` 주석으로 남긴다 — 주석의 형식과 목적은 [컨벤션 문서](docs/conventions/conventions.md) 도입부가 규정한다.

| 결정                                                                  | 근거                                                                                                                                                                                                                          | 위치                                                                |
| --------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| 비밀번호를 "영문·숫자 **외 문자 불가**"로 해석 (영문만/숫자만도 통과) | 요구사항 문구 "영문, 숫자로 구성된"은 "둘 다 포함"으로도 읽히나, 스키마의 `^[A-Za-z0-9]+$` 를 우선했다                                                                                                                        | `pages/sign-in/ui/sign-in-page.tsx`                                 |
| 삭제 확인 입력의 앞뒤 공백 허용                                       | id 를 화면에서 복사해 붙여넣는 경로를 고려. "동일한 값"의 엄격 해석은 아니지만 확인 절차의 목적은 유지된다                                                                                                                    | `features/task-delete/ui/delete-task-dialog.tsx`                    |
| 로그아웃 기능과 `POST /api/sign-out` 추가 (명세에 없음)               | 세션을 수동 종료할 수단이 없으면 30초 TTL 의 401 → refresh → 재시도 경로를 검수하기 어렵다. 실제 환경의 refresh 쿠키는 `HttpOnly` 라 클라이언트가 지울 수 없으므로 서버 무효화 엔드포인트가 유일하게 올바른 로그아웃 방식이다 | `mocks/handlers/auth.ts`, `entities/session/api/auth-repository.ts` |
| sign-in 응답 본문의 `refreshToken` 을 저장하지 않음                   | JS 로 영속화하면 XSS 노출면이 넓어진다. 서버가 `Set-Cookie` 로 심는 것을 전제했으나 **명세의 sign-in 응답에는 이 규정이 없어 백엔드 계약 협의가 필요한 가정**이다                                                             | `entities/session/model/session-store.ts`                           |
| accessToken 은 메모리에만 보관                                        | 같은 이유로 `localStorage` 를 쓰지 않는다. 새로고침 시에는 refresh 쿠키로 세션을 복원한다                                                                                                                                     | `shared/api/token-store.ts`                                         |
| 프로덕션 빌드에서도 MSW 기동                                          | 실제 백엔드가 없어 `pnpm preview` 결과물이 그대로 동작해야 한다. 목 서버를 개발 보조가 아니라 배포 결과물의 백엔드로 취급했다                                                                                                 | `app/main.tsx`                                                      |
| `GET /api/task` 의 `page` 누락·비정수 시 400 (명세에 없는 상태코드)   | 명세가 `page` 를 필수로 정의하므로 목 서버도 검증한다. 앱은 항상 정수를 보내므로 화면 동작에는 영향이 없다                                                                                                                    | `mocks/handlers/task.ts`                                            |
| 회원정보 라우트를 `/user` 로 지정                                     | 요구사항이 화면만 명시하고 경로를 주지 않았다. 다른 라우트가 API 경로와 같은 이름을 쓰므로 맞췄다                                                                                                                             | `routes/_app.user.tsx`                                              |
| `primary: blue` 를 색상 강제가 아닌 예시로 해석                       | 요구사항의 의도는 "색상을 토큰으로 관리하라"이고 값 자체는 예시로 읽었다. shadcn `baseColor` 는 `olive` 를 쓴다                                                                                                               | [스타일 정책](#스타일-정책)                                         |
| 목록 카드에 `status` 배지 표시                                        | 요구사항은 title·memo 만 요구하나 `TaskItem` 스키마에 `status` 가 있어, 대시보드의 "해야할 일/한 일" 집계와 목록을 눈으로 대조할 수 있게 했다                                                                                 | `entities/task/ui/task-card.tsx`                                    |

목 서버가 실제 환경과 다르게 타협한 지점(토큰 TTL, 서명 없는 JWT, 쿠키 플래그 등)은 [컨벤션 문서 3.4절](docs/conventions/conventions.md)에 표로 정리했고, 코드의 `//- 프로덕션과 다름.` 주석과 개수가 일치하는지 `pnpm lint` 가 검사한다.
...
서버 데이터는 `entities/*/api`의 **Repository**(HTTP 호출)와 `entities/*/model`의 **Queries**(캐시 정책)로 나누고, 뮤테이션은 `features/`의 훅에 둔다. 각 구성 요소의 확정 규칙(작성 형태, 쿼리 키, 캐시 갱신 정책 등)은 [컨벤션 문서 2절](docs/conventions/conventions.md#2-데이터-레이어-컨벤션)이 규정한다.
